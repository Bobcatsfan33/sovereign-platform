...
| --- | --- | --- | --- |
| **SI-2** Flaw Remediation | implemented | Container image vulnerabilities are flagged by the `trivy` job in CI. The job fails on any critical/high finding before images are pushed, signed, or attested. | `.github/workflows/ci.yml::trivy-scan` |
| **SI-2(2)** Automated Flaw Remediation Status | implemented | Trivy scan results upload as a CI artefact every run; the continuous monitor includes "all images scanned within window" in its dashboard. | `scripts/continuous_monitor.py::check_image_scan_freshness` |
| **SI-3** Malicious Code Protection | inherited | Endpoint protection runs on the agency-managed container host (organizational). |  |
| **SI-4** System Monitoring | implemented | The audit-service's GET /events API + portal Compliance dashboard provide real-time visibility into every chassis action. The continuous monitor (5.2) drives automated alerts. | `apps/portal/src/pages/Compliance.tsx`; `scripts/continuous_monitor.py` |
...
| **SI-7** Software, Firmware, and Information Integrity | implemented | Container images are content-addressable, scanned before push, signed with cosign, attested with build provenance on main, and verified at admission by the Kyverno overlay. The OPA policy bundle is mounted read-only. Audit rows are append-only by API design (no DELETE endpoint). | `.github/workflows/ci.yml`; `deploy/k8s/admission-cosign-policy.yaml`; `apps/*/Dockerfile`; `docker-compose.yml::opa::volumes` |
...
| Additional | Note |
| --- | --- |
| SI-7(1)+(6) Integrity Checks + Cryptographic Protection | Images are signed in CI and the Kyverno overlay verifies signatures, digests, and SLSA provenance at admission. |
