...
| **SI-7** Software, Firmware, and Information Integrity | implemented (in part) | Container images are content-addressable, scanned before push, signed with cosign, and attested with build provenance on main. The OPA policy bundle is mounted read-only. Audit rows are append-only by API design (no DELETE endpoint). | `.github/workflows/ci.yml`; `apps/*/Dockerfile`; `docker-compose.yml::opa::volumes` |
