...
| **SC-13** Cryptographic Protection | implemented | See SC-8(1). FIPS 140-2/140-3 enforcement is policy-based and verified by `policies/tests/crypto_test.rego::test_legacy_rc4_denies`. The chassis itself uses Python's `hashlib` + `secrets` + `hmac` (all FedRAMP-compliant via the OpenSSL FIPS module bundled with the agency base image — POA&M 5.4-A documents the switch to FIPS-Python). | `policies/base/crypto.rego` |
