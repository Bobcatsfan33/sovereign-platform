...
| **AC-7** Unsuccessful Logon Attempts | implemented (front door + HTTP-layer) | The broker returns bounded 401 responses for failed auth and the Kubernetes front-door overlay applies RPS, burst, connection, TLS, and body-size limits before requests reach the broker. JWT verification failures land in the audit trail via `identify`. | `apps/broker/app/main.py::identify`; `deploy/k8s/frontdoor-rate-limit.yaml` |
