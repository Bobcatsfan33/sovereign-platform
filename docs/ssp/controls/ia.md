...
| **IA-2(1)** MFA for Privileged Accounts | inherited | Enforced by the agency IdP. The chassis itself has no concept of "MFA satisfied" beyond what the IdP asserts in the `amr` claim, which is logged in the audit metadata for `policy.evaluated`. |  |
| **IA-2(2)** MFA for Non-Privileged Accounts | inherited | Same as IA-2(1). |  |
| **IA-2(8)** Replay-Resistant Authentication | implemented | JWTs include `iat` + `exp`; the verifier rejects tokens past `exp`. Phase 3.5 OIDC `nonce` enforcement covers the OIDC implicit-flow path (POA&M 5.2-F for further hardening). |  |
...
| Additional | Note |
| --- | --- |
| IA-2(11) MFA for Remote Access | Mandate MFA via the IdP for any session whose `amr` claim doesn't include `mfa`. Add Rego rule in `sovereign.base` (POA&M 5.2-G). |
