...
| **IA-2(8)** Replay-Resistant Authentication | implemented | JWTs include `iat` + `exp`; the verifier rejects tokens past `exp`. The portal OIDC callback validates `state`, `nonce`, and `exp` before storing a bearer token. JWKS cache refresh honors IdP `max-age` and uses bounded stale keys during short IdP outages. | `libs/common/sovereign/idp/oidc.py`; `apps/portal/src/auth/oidc.ts`; `tests/test_idp.py`; `apps/portal/src/test/oidc.test.ts` |
