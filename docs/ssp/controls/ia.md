...
| **IA-2(8)** Replay-Resistant Authentication | implemented | JWTs include `iat` + `exp`; the verifier rejects tokens past `exp`. Phase 3.5 OIDC `nonce` enforcement covers the OIDC implicit-flow path (POA&M 5.2-F for further hardening). JWKS cache refresh honors IdP `max-age` and uses bounded stale keys during short IdP outages. | `libs/common/sovereign/idp/oidc.py`; `tests/test_idp.py` |
