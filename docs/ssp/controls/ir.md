...
| **IR-5** Incident Monitoring | implemented | The continuous monitor pages on-call when audit gaps, policy drift, or state drift exceed agency-defined thresholds. ClickHouse aggregations in the portal Compliance dashboard surface anomalies (denies per principal, denies per service type). | `scripts/continuous_monitor.py`; `apps/portal/src/pages/Compliance.tsx` |
